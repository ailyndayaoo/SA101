...
function Dtr() {
  ...
  // helper: pad and validate time parts
  const pad2 = (s) => ('' + s).padStart(2, '0');
  const isValidTimePart = (hours, minutes) => {
  ...
  const makeTimeString = (hours, minutes, period) => `${pad2(hours)}:${pad2(minutes)} ${period}`;

  useEffect(() => {
    const selectedBranch = localStorage.getItem('selectedBranch');

    fetch('http://localhost:3001/active_staff?branch=' + selectedBranch)
      .then((response) => response.json())
      .then((data) => {
        // ensure each employee has timeIn/timeOut objects
        const mapped = (data || []).map(emp => ({
          ...emp,
          timeIn: emp.timeIn || { hours: '08', minutes: '00', period: 'AM' },
          timeOut: emp.timeOut || { hours: '05', minutes: '00', period: 'PM' }
        }));
        setEmployees(mapped);
      })
  ...
  const handleSave = (employeeId) => {
    ...
    const inH = (employee.timeIn?.hours ?? '').toString();
    const inM = (employee.timeIn?.minutes ?? '').toString();
    const inP = (employee.timeIn?.period ?? 'AM').toString().toUpperCase();

    const outH = (employee.timeOut?.hours ?? '').toString();
    const outM = (employee.timeOut?.minutes ?? '').toString();
    const outP = (employee.timeOut?.period ?? 'PM').toString().toUpperCase();

    if (!isValidTimePart(inH, inM) || !['AM','PM'].includes(inP)) {
      return alert('Invalid Time In. Enter hours 1-12 and minutes 00-59, then choose AM/PM.');
    }
    if (!isValidTimePart(outH, outM) || !['AM','PM'].includes(outP)) {
      return alert('Invalid Time Out. Enter hours 1-12 and minutes 00-59, then choose AM/PM.');
    }

    const timeIn = makeTimeString(inH, inM, inP);
    const timeOut = makeTimeString(outH, outM, outP);
    ...
    fetch('http://localhost:3001/save_dtr', {
      method: 'POST',
      headers: {
